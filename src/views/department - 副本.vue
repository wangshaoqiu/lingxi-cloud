...
<template slot-scope="{row}">
    ...
    <td>
        <badge class="badge-dot mr-4" :type="row.statusType">
            <span class="status">{{row.createTimeStr}}</span>
        </badge>
    </td>
    <td>
        {{row.fileVizTypeStr}}
    ...
    <td>
        <!-- <div>
            <el-tooltip content="编辑" placement="top">
                <base-button type="warning" v-if="row.type==1" @click="showEditDialog(row.dofId,1)">
                    <i class="el-icon-edit"></i>
                </base-button>
                <base-button type="info" v-if="row.type==2" @click="showEditDialog(row.dofId,2)">
                    <i class="ni ni-key-25"></i>
                </base-button>
            </el-tooltip>
        </div>
        <div class="ml10" @click="showDelDialog(row.dofId,row.type)">
        ...
        <div class="color999 font15 ml10">编辑</div>
        </div>
        ...
        <div class="color999 font15 ml10">删除</div>
        </div>
</div>
    </td>
</template>
            ...
            handleSuccess(res, file) {
                for (let i = 0; i < this.uploadStrategy.files.length; i++) {
                    if (this.uploadStrategy.files[i].name == file.name) {
                        ...
                        this.uploadStrategy.files[i].url = this.uploadStrategy.fsDomain + file.response.hash
